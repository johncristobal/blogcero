		...
		<table class="form" border="0">
			...
			<tr>
				<td>Seleccionar alumno:</td>
                                ...
                                <select id="estudianteA" name="estudianteA" onchange="verexpedientes();" style="width: 20em;">
                                    <option value="ninguno">Seleccionar</option>
                                    @foreach($alumnos as $value)
                                        @if($value->id == $alumno_elegido)
                                            <option value="{{$value->id}}" selected="true">{{$value->name}} {{$value->pat}} {{$value->mat}}</option>
                                        @else
                                            <option value="{{$value->id}}">{{$value->name}} {{$value->pat}} {{$value->mat}}</option>
                                        @endif
                                    @endforeach
                                </select>
			...
			<tr><td colspan="3" align="center"><input type="submit" value="Reasignar"></td></tr-->
                ...
                <table width="100%" id="tabledata">
                    ...
                    <tbody id="cuerpotabla">
                        ...
                        ?>
                        <tr>
                            <td width='50%'><?php echo "$value->folio_expediente - $value->nombre_paciente $value->ap_paterno";?></td>
                            <td>
                                <select name="nuevo<?php echo $i;?>" style="width: 20em;">
                                    <option value="ninguno">Seleccionar</option>
                                    @foreach($alumnos as $value)
                                        <option value="{{$value->id}}">{{$value->name}} {{$value->pat}} {{$value->mat}}</option>
                                    @endforeach
